...
export function RoleChoiceCard({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "group flex min-h-[280px] flex-col rounded-[2rem] border-2 border-border bg-card p-6 text-left shadow-[0_20px_60px_-36px_rgba(26,26,26,0.22)] transition-all",
    ...
    >
      ...
      <div className="space-y-3">
        ...
        <h2 className="font-display text-3xl font-bold tracking-tight text-foreground md:text-[2.1rem]">
          {title}
        </h2>
      ...
      <div className="mt-auto pt-8 md:pt-10">
        <span className="inline-flex items-center rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition-transform group-hover:translate-x-1">
          {ctaLabel}
        </span>
      </div>
    </button>
