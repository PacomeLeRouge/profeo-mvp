...
export default function DevHubPage() {
  ...
  return (
    <div className="min-h-screen bg-white px-6 py-16 text-foreground md:px-12">
      <div className="mx-auto max-w-3xl space-y-10">
        <div className="space-y-3">
          ...
          <p className="text-lg leading-8 text-muted-foreground">
            ...
            <code className="rounded bg-primary/5 px-1.5 py-0.5 text-base">npm run dev</code>.
          </p>
        </div>
        ...
        <div className="grid gap-4">
          {previews.map((preview) => (
            <Link
              key={preview.href}
              href={preview.href}
              className="group rounded-3xl border border-border p-6 transition-colors hover:border-primary/30 hover:bg-muted"
            >
              <h2 className="text-2xl font-semibold tracking-tight group-hover:underline">
                {preview.title}
              </h2>
              <p className="mt-2 text-base leading-7 text-muted-foreground">{preview.description}</p>
              <p className="mt-4 text-sm font-medium text-foreground/70">{preview.href}</p>
            </Link>
          ))}
        </div>
      </div>
